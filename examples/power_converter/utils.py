...
def get_thd(y, t, freq):
    ...
    if y.shape[1] != 3:
        raise ValueError(
            'Wrong dimensions of y. y must be 3 x N where N is the number of samples')
...
def get_dft(signal, time, freq):
    ...
    Note:
        It is used to compute the time window which is a multiple of the
        fundamental period thus ensuring that the frequency spectrum is 'sharp'
        and not smeared out.
    """
